...
**Goal:** Replace the placeholder `app/terms/page.tsx` with a full, production-ready Terms of Service page covering both the Thesis AI Platform and Long Hold iOS app, using a sticky TOC sidebar layout.

**Architecture:** Single server component page (`app/terms/page.tsx`) with no client-side JS — sticky sidebar uses CSS `position: sticky`, TOC links use native anchor navigation. A minimal `.tos-sidebar` CSS rule added to `app/globals.css` handles mobile hiding. All styling via existing design tokens and inline styles, matching the Long Hold privacy page pattern.

**Tech Stack:** Next.js 15 App Router, React 19, TypeScript, CSS design tokens (`var(--bg-soft)` etc.), Tailwind (for base reset only)
...
|---|---|---|
| Modify | `app/terms/page.tsx` | Full ToS page — metadata, header, sticky TOC, 16 content sections, footer |
| Modify | `app/globals.css` | Add `.tos-sidebar` media query to hide TOC on mobile |

---

## Task 1: Page scaffold, header, and metadata

**Files:**
- Modify: `app/terms/page.tsx`

- [ ] **Step 1: Replace `app/terms/page.tsx` with scaffold**
...
git commit -m "feat: terms page scaffold with header and metadata"
```

---

## Task 2: Sticky TOC sidebar + responsive CSS

**Files:**
- Modify: `app/terms/page.tsx`
- Modify: `app/globals.css`

- [ ] **Step 1: Add `.tos-sidebar` responsive rule to `app/globals.css`**

Add at the end of `app/globals.css`:

```css
/* ─────────────────────────────────────────
   Terms of Service — TOC sidebar
──────────────────────────────────────────── */
...
- [ ] **Step 2: Add content area with sticky TOC to `app/terms/page.tsx`**

Insert the following block between the closing `</div>` of the page header and the `<footer>` tag:
...
git commit -m "feat: terms page sticky TOC sidebar with responsive CSS"
```

---

## Task 3: Sections 1–4 (Acceptance, Services, Platform Beta, Long Hold Billing)

**Files:**
- Modify: `app/terms/page.tsx`

- [ ] **Step 1: Replace the empty `<div style={{ flex: 1, minWidth: 0 }}>` with sections 1–4**
...
```bash
git add app/terms/page.tsx
git commit -m "feat: terms sections 1-4 (acceptance, services, beta, billing)"
```

---

## Task 4: Sections 5–9 (Accounts through AI-Generated Content)

**Files:**
- Modify: `app/terms/page.tsx`

- [ ] **Step 1: Add sections 5–9 inside the `<div style={{ flex: 1, minWidth: 0 }}>` block, after section 4 and before the closing `</div>`**
...
```bash
git add app/terms/page.tsx
git commit -m "feat: terms sections 5-9 (accounts, use, IP, financial disclaimer, AI content)"
```

---

## Task 5: Sections 10–16 + footer

**Files:**
- Modify: `app/terms/page.tsx`

- [ ] **Step 1: Add sections 10–16 inside the `<div style={{ flex: 1, minWidth: 0 }}>` block, after section 9 and before the closing `</div>`**
...
Expected: no TypeScript errors, successful Next.js build with `/terms` listed in the output

- [ ] **Step 3: Commit**

```bash
git add app/terms/page.tsx
git commit -m "feat: terms sections 10-16 (privacy through contact) — complete ToS page"
```
